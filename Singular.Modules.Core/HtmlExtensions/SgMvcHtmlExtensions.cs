    ...
    public static class SgMvcHtmlExtensions
    {
            ...
            Expression<Func<TModel, TProperty>> propertyExpression, Action<NgEditorBuilder> func)
        {
            ...
            if (model.IsNotNull())
            {
                ...
                {
                    output.AppendFormat("<div class=\"form-group{0}\">",
                        edBuilder.Editor == "UiRadioList" ? " uiradio-formgroup" : "");
                ...
                switch (edBuilder.Editor)
                {
                    ...
                    case "UiCheckbox":
                    {
                        ...
                        output.AppendFormat(
                            ...
                            propName,
                            propName.Replace(".", "_"),
                            edBuilder.UiCheckboxLabelPosition != "left" && edBuilder.UiCheckboxLabelPosition != "right"
                                ? "display:block;"
                                : "",
                ...
                // add error
                output.AppendFormat(
                    "<{2} style=\"color:red; font-size: 9px; padding-{4}: 8px;{5}\" sg-property-error=\"{3}['{0}{1}']\"></{2}>",
                    edBuilder.ModelPrefixValue.HasValue() ? edBuilder.ModelPrefixValue + "." : "Model.", propName,
                    edBuilder.IsBootstrapFormGroup ? "div" : "span",
                    edBuilder.ValidationErrorsPropertyNameValue ?? "SgValidationErrors",
                    edBuilder.IsBootstrapFormControl ? "bottom" : "left",
                    edBuilder.IsBootstrapFormControl ? "margin-top: -10px;" : "");
        ...
        /// <returns></returns>
        public static MvcHtmlString SgValidationErrors(this HtmlHelper html, string scopePropertyName,
